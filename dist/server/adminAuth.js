...
export async function createAdminSession(userId, ipAddress, userAgent) {
...
export async function validateAdminSession(token) {
    ...
    const [session] = await db
        ...
        .where(and(eq(adminSessions.token, hashedToken), gt(adminSessions.expiresAt, new Date())));
    if (!session)
        return null;
...
export async function promoteUserToAdmin(userId, role = USER_ROLES.ADMIN, permissions = []) {
