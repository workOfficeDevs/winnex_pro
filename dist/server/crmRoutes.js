...
export function registerCrmRoutes(app) {
    ...
    app.get("/api/crm/users", requirePermission('access_crm'), async (req, res) => {
        try {
            ...
            let users;
            if (search) {
                users = await crmService.searchUsers(search, parseInt(limit));
            }
            else if (segment && segment !== 'all') {
                users = await crmService.getUsersBySegment(segment, parseInt(limit));
            }
            else {
                // Return recent users if no specific search
                users = await crmService.getUsersBySegment('all', parseInt(limit));
            }
            res.json(users);
        }
        catch (error) {
    ...
    app.get("/api/crm/risk-alerts", isAuthenticated, async (req, res) => {
        try {
            ...
            const alerts = await crmService.getRiskAlerts(userId, severity, resolved === 'true');
            res.json(alerts);
        }
        catch (error) {
    ...
    app.get("/api/crm/transactions", isAuthenticated, async (req, res) => {
        try {
            ...
            if (flagged === 'true') {
                ...
                const transactions = await crmService.getTransactionHistory(userId);
                res.json(transactions);
            }
            else {
    ...
    app.post("/api/crm/notifications", isAuthenticated, async (req, res) => {
        try {
            ...
            const result = await crmService.sendNotificationToUsers(userIds, channel, subject, content, campaignName);
            const adminId = req.user?.claims?.sub || 'admin';
            await crmService.logAdminAction({
                adminId,
