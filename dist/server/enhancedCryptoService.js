export class EnhancedCryptoService {
        ...
        try {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    const majorCoins = data.filter((coin) => ['BTC', 'ETH', 'USDT', 'LTC', 'DOGE'].includes(coin.asset_id));
                    return majorCoins.map((coin) => ({
            ...
            try {
                ...
                if (response.ok) {
                    const data = await response.json();
                    const relevantPairs = data.filter((ticker) => ['BTCUSDT', 'ETHUSDT', 'LTCUSDT', 'DOGEUSDT'].includes(ticker.symbol));
                    const enhanced = relevantPairs.map((ticker) => ({
    ...
    async verifyEthereumTransaction(txHash) {
        try {
    ...
    async generateSecureWalletAddress(userId, currency) {
        try {
    ...
    async getMarketSentiment() {
        try {
            ...
            const response = await fetch(`https://min-api.cryptocompare.com/data/blockchain/mining/calculator?fsyms=BTC,ETH&tsyms=USD&api_key=${this.cryptoCompareKey}`);
            if (response.ok) {
                const data = await response.json();
