...
export class OddsMarginEngine extends EventEmitter {
    ...
    async processMarginAdjustments() {
        try {
            ...
            const activeOdds = await db
                ...
                .where(and(eq(enhancedOdds.isActive, true), eq(enhancedMatches.status, 'scheduled')));
            const adjustments = [];
    ...
    async calculateMarginAdjustment(odds, match) {
    ...
    async getDynamicAdjustments(odds, match, config) {
    ...
    async getMarginAnalytics() {
        try {
            const analytics = await db
                ...
                avgMargin: sql `AVG(${enhancedOdds.margin})`,
                minMargin: sql `MIN(${enhancedOdds.margin})`,
                maxMargin: sql `MAX(${enhancedOdds.margin})`,
                totalVolume: sql `SUM(${enhancedOdds.volume})`,
                count: sql `COUNT(*)`
            })
                .from(enhancedOdds)
