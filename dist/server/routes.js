...
export async function registerRoutes(app) {
    ...
    app.get('/api/matches', async (req, res) => {
        try {
            ...
            const filteredMatches = sportId
                ? processedMatches.filter(match => match.sportId === parseInt(sportId))
                : processedMatches;
    ...
    app.post('/api/odds', isAuthenticated, async (req, res) => {
        try {
            const userId = req.user.claims.sub;
            const user = await storage.getUser(userId);
            if (!user?.isAdmin) {
    ...
    app.get("/api/compliance/status", isAuthenticated, async (req, res) => {
        try {
            const userId = req.user.claims.sub;
            const user = await storage.getUser(userId);
    ...
    app.post('/api/crypto/deposit', isAuthenticated, async (req, res) => {
        try {
            ...
            const user = await storage.getUser(userId);
            if (user?.email) {
                await notificationService.sendDepositConfirmation(user.email, transaction.amount, currency, txHash);
            }
    ...
    app.post('/api/crypto/withdraw', isAuthenticated, async (req, res) => {
        try {
            ...
            const user = await storage.getUser(userId);
            if (user?.email) {
                await notificationService.sendWithdrawalAlert(user.email, amount, currency, toAddress);
            }
            res.json(transaction);
        }
        catch (error) {
