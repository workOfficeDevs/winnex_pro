...
export async function seedCrmData() {
  try {
    ...
    const profilesData = existingUsers.map((user, index) => ({
      ...
      kycStatus: ['basic', 'intermediate', 'advanced'][index % 3] as any,
      userSegment: ['new', 'casual', 'vip', 'high_roller'][index % 4] as any,
      riskLevel: ['low', 'medium', 'high'][index % 3] as any,
      complianceStatus: 'clear' as any,
    ...
    const ticketsData = existingUsers.slice(0, 3).map((user, index) => ({
      ...
      status: ['open', 'in_progress', 'resolved'][index] as any,
      priority: ['medium', 'high', 'low'][index] as any,
    ...
    const walletsData = existingUsers.slice(0, 5).flatMap((user, userIndex) => 
      ['BTC', 'ETH', 'USDT'].map((currency, currencyIndex) => ({
        userId: user.id,
        currency,
    ...
    const transactionsData = existingUsers.slice(0, 5).flatMap((user, userIndex) => 
      Array.from({ length: 3 }, (_, transIndex) => ({
        userId: user.id,
        type: ['deposit', 'withdrawal', 'bet', 'win'][transIndex % 4] as any,
        currency: ['BTC', 'ETH', 'USDT'][transIndex % 3],
        amount: (Math.random() * 5).toFixed(8),
        usdValue: (Math.random() * 1000).toFixed(2),
        status: ['confirmed', 'pending', 'confirmed'][transIndex % 3] as any,
    ...
    const affiliatesData = existingUsers.slice(0, 2).map((user, index) => ({
      ...
      affiliateType: ['referral', 'partner'][index] as any,
      commissionRate: '0.05',
