...
export class CryptoService {
  ...
  generateDepositAddress(userId: string, currency: string): string {
  ...
  // Calculate USD value
  async calculateUsdValue(amount: number, currency: string): Promise<number> {
  ...
  async verifyTransaction(txHash: string, currency: string): Promise<{
    confirmed: boolean;
    amount: number;
    toAddress: string;
  }> {
  ...
  async processDeposit(userId: string, txHash: string, currency: string, expectedAddress: string): Promise<CryptoTransaction> {
  ...
  async processWithdrawal(userId: string, amount: number, currency: string, toAddress: string): Promise<CryptoTransaction> {
  ...
  async getUserBalances(userId: string): Promise<CryptoBalance[]> {
  ...
  async convertToCredits(amount: number, currency: string): Promise<number> {
    const usdValue = await this.calculateUsdValue(amount, currency);
    return usdValue; // 1 USD = 1 betting credit
  }

  // Convert betting credits to crypto
  async convertFromCredits(credits: number, currency: string): Promise<number> {
    const price = await this.getPrice(currency);
