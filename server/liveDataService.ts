...
export class LiveDataService {
  ...
  private sportsApiKey = 'dex_18bc864938355c60df837ad5328026fd';
  private sportsApiUrl = 'https://api.sportsdata.io/v3';
  
  // Enterprise Market Data API
  private marketDataKey = 'dex_3a943819ad386569d048b4270854bbd3';
  private marketDataSecret = '24395d132ad626485d044aa3efd0df91ac5a6f40e38889566b0841725b2285f2';
  
  // Trading API
  private tradingApiKey = 'dex_5eef12832b83ecf55e808fc7c54a0a1d';
  private tradingApiSecret = 'd4c90d91666cebfb8f2e0fd6724165d67270e7d67ccc5059800f42636a45cd4c';
  ...
  async getLiveMatches(): Promise<LiveSportsData[]> {
    const results: LiveSportsData[] = [];
  ...
  async getUpcomingMatches(): Promise<LiveSportsData[]> {
    const results: LiveSportsData[] = [];
  ...
  private normalizeApiResponse(data: any, provider: string): LiveSportsData[] {
    const matches: LiveSportsData[] = [];
      ...
      switch (provider) {
        ...
        case 'sportradar':
          if (data.sport_events) {
            data.sport_events.forEach((event: any) => {
              matches.push({
                id: event.id,
                sport: 'Soccer',
  ...
  private normalizeSportsData(data: any[]): LiveSportsData[] {
  ...
  private normalizeMarketData(data: any[]): LiveSportsData[] {
    return data.map((event: any) => ({
      ...
      odds: event.bookmakers?.[0]?.markets?.[0]?.outcomes ? {
        home: event.bookmakers[0].markets[0].outcomes.find((o: any) => o.name === event.home_team)?.price || 1.85,
        away: event.bookmakers[0].markets[0].outcomes.find((o: any) => o.name === event.away_team)?.price || 2.10,
        draw: event.bookmakers[0].markets[0].outcomes.find((o: any) => o.name === 'Draw')?.price
      } : { home: 1.85, away: 2.10 }
    }));
  }

  private mapGameStatus(status: string): 'live' | 'upcoming' | 'finished' {
    if (!status) return 'upcoming';
    const lowerStatus = status.toLowerCase();
  ...
  private getRealisticFallbackData(): LiveSportsData[] {
  ...
  private getRealisticUpcomingData(): LiveSportsData[] {
    const currentTime = new Date();
