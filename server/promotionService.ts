...
export class PromotionService extends EventEmitter {
  private validationRules: Map<string, (userId: string, promotion: any, data?: any) => Promise<PromoValidation>> = new Map();
  ...
  private initializeValidationRules() {
    ...
    this.validationRules.set('welcome_bonus', async (userId: string, promotion: any) => {
      ...
      const existingBonus = await db
        ...
        .where(
          and(
            ...
            eq(userPromotions.promotionId, promotion.id)
          )
        )
        .limit(1);
    ...
    this.validationRules.set('deposit_match', async (userId: string, promotion: any, data?: { depositAmount: number }) => {
    ...
    // Free bet validation
    this.validationRules.set('free_bet', async (userId: string, promotion: any) => {
      // Check usage limit
    ...
    this.validationRules.set('odds_boost', async (userId: string, promotion: any, data?: { betAmount: number, odds: number }) => {
  ...
  public async createPromotion(promotionData: {
    ...
    title: string;
    description: string;
    type: 'welcome_bonus' | 'deposit_match' | 'free_bet' | 'odds_boost';
    value: number;
    minDeposit?: number;
    maxBonus?: number;
    validFrom: Date;
  ...
  public async applyPromotionCode(userId: string, code: string, data?: any): Promise<PromoValidation> {
    try {
      ...
      const promotion = await db
        ...
        .where(
          and(
            ...
            gte(promotions.validTo, new Date())
          )
        )
        .limit(1);
      ...
      const existingUsage = await db
        ...
        .where(
          and(
            eq(userPromotions.userId, userId),
            eq(userPromotions.promotionId, promotion[0].id)
          )
        )
        .limit(1);
  ...
  private async grantPromotion(userId: string, promotion: any, bonusAmount: number) {
    try {
  ...
  public async createReferralLink(userId: string): Promise<string> {
  ...
  public async processReferralSignup(refereeId: string, referralCode: string): Promise<ReferralReward | null> {
    try {
  ...
  public async completeReferral(refereeId: string, qualifyingAmount: number = 50) {
    try {
      const referral = await db
        .select()
        .from(referrals)
  ...
  public async getUserPromotions(userId: string) {
    try {
  ...
  public async getAvailablePromotions(userId?: string) {
    try {
      ...
      let availablePromos = await db
        ...
        .where(
          and(
            ...
            gte(promotions.validTo, currentDate)
          )
        );
  ...
  public async getPromotionAnalytics(promotionId?: number) {
    try {
      if (promotionId) {
        ...
        const analytics = await db
          .select({
            ...
            totalBonusAmount: sql<number>`SUM(${userPromotions.bonusAmount})`,
            activeUsers: sql<number>`COUNT(DISTINCT ${userPromotions.userId})`
          })
          .from(userPromotions)
  ...
  private async getReferrerFromCode(referralCode: string): Promise<string | null> {
  ...
  public async deactivatePromotion(promotionId: number) {
    try {
      await db
        .update(promotions)
        .set({ isActive: false })
