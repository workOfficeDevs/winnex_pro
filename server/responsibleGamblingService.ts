...
export class ResponsibleGamblingService extends EventEmitter {
  ...
  private async performLimitChecks() {
    try {
  ...
  // Set user limits
  public async setUserLimits(userId: string, limits: {
  ...
  public async checkDepositLimit(userId: string, amount: number): Promise<LimitCheck> {
  ...
  public async checkBetLimit(userId: string, amount: number): Promise<LimitCheck> {
    const limits = await this.getUserLimits(userId);
    if (!limits) {
      return {
        userId,
  ...
  // End user session
  public async endSession(userId: string) {
  ...
  public async checkSessionLimit(userId: string): Promise<boolean> {
    const session = this.activeSessions.get(userId);
    if (!session) return true; // No active session

    const limits = await this.getUserLimits(userId);
    if (!limits || !limits.sessionTimeLimit) return true; // No time limit set
  ...
  public updateSessionBet(userId: string, betAmount: number) {
    const session = this.activeSessions.get(userId);
    if (session) {
      session.betsPlaced++;
  ...
  public async getUserLimits(userId: string) {
    try {
  ...
  private async getDepositSum(userId: string, period: 'daily' | 'weekly' | 'monthly'): Promise<number> {
  ...
  private async getBetSum(userId: string, period: 'daily' | 'weekly' | 'monthly'): Promise<number> {
    const startDate = this.getPeriodStartDate(period);
    
    try {
      const result = await db
        .select({
          total: sql<number>`COALESCE(SUM(${enhancedBets.stake}), 0)`
        })
        .from(enhancedBets)
        .where(
          and(
            eq(enhancedBets.userId, userId),
  ...
  public async getLimitStatus(userId: string) {
    try {
      const limits = await this.getUserLimits(userId);
